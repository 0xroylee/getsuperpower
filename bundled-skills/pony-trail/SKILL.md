...
Use `scripts/snapshot_change.sh` from this skill directory. It writes JSONL under `.omniskills/`, stores file hashes plus small before/after copies, and appends a per-session git-like tree under `.omniskills/sessions/<session-id>/tree.md` without requiring Python. `scripts/snapshot_change.py` remains available for older environments that already call it.
