...
Omniskill 是給 AI agents 使用的 many-skill bank：安裝一個 workflow skill
...
<img src="/assets/omniskill.jpg" alt="Omniskill" width="640" />
...
使用 registry view 挑選一個 Omniskill workflow、檢視它的 role workflow，並複製 install command。

<img src="assets/omniskill-startup-role-registry.png" alt="Omniskill startup role workflow registry" width="920" />

## Startup Role Workflows

| Omniskill workflow | Entry skill | 用途 |
| --- | --- | --- |
...
有些 Omniskill workflows 也提供 loop runner，適合需要持續推進直到完成的 goals。loop 是可恢復的 workflow state：
...
Omniskill workflows 可以組合 local skills、bundled skills、external skill packs：
...
如果 automatic bootstrap 失敗，請透過 Omniskill 執行 package install，然後重試：
...
```text
$creating-bundle-skills create an Omniskill workflow for release review
```
...
除非你明確想分享 installed workflow records，否則請不要把 project-local `.omniskill/` folders 加進 git。
...
Canonical package 和 CLI binary 的名稱是 `omniskill`；舊的 `omniskill`
