...
describe("omniskill command module", () => {
  test("registers Omniskill commands and dependency aliases", () => {
  ...
  test("install supports a public git workflow source", async () => {
    ...
    const installed = JSON.parse(
      await readFile(join(rootDir, ".omniskill", "workflows", "git-workflow.json"), "utf8"),
    );
  ...
  test("install writes the workflow record to the global home by default", async () => {
    ...
    await expect(
      stat(join(homeDir, ".omniskill", "workflows", "git-workflow.json")),
    ).resolves.toBeTruthy();
    await expect(
      stat(join(rootDir, ".omniskill", "workflows", "git-workflow.json")),
    ).rejects.toThrow();
  ...
  test("install force-refreshes only recorded workflow artifacts when workflow version changes", async () => {
    ...
    await mkdir(join(rootDir, ".omniskill", "workflows"), { recursive: true });
    await writeFile(
      join(rootDir, ".omniskill", "workflows", "git-workflow.json"),
      JSON.stringify(
        {
          schemaVersion: "0.1",
          name: "git-workflow",
  ...
  test("install persists exact skill artifact paths in the workflow record", async () => {
    ...
    try {
      ...
      const installed = JSON.parse(
        await readFile(join(homeDir, ".omniskill", "workflows", "git-workflow.json"), "utf8"),
      );
  ...
  test("remove dry-run prints the plan without deleting artifacts or workflow record", async () => {
    ...
    try {
      ...
      expect(stripAnsiLines(logs).join("\n")).toContain("Omniskill remove plan: git-workflow");
  ...
  test("remove confirmed with yes deletes artifacts and workflow record", async () => {
    ...
    try {
      ...
      expect(stripAnsiLines(logs)).toContain("Omniskill removed: git-workflow");
  ...
  test("remove keeps artifacts still referenced by another workflow", async () => {
    ...
    try {
      ...
      await expect(
        stat(join(homeDir, ".omniskill", "workflows", "ops-workflow.json")),
      ).resolves.toBeTruthy();
    } finally {
      console.log = originalLog;
      await rm(rootDir, { recursive: true, force: true });
  ...
  test("install asks for approval with the declared skill list before installing", async () => {
    ...
    try {
      ...
      expect(stripAnsiLines(logs)).toContain("Omniskill install cancelled.");
      expect(skillInstalls).toEqual([]);
      await expect(
        stat(join(homeDir, ".omniskill", "workflows", "git-workflow.json")),
      ).rejects.toThrow();
  ...
  test("install shows progress and a branded OMNISKILL result when approved", async () => {
    ...
    try {
      ...
      expect(output).toContain("OMNISKILL");
      expect(output).toContain("Omniskill installed: git-workflow");
      expect(output).toContain(
        `Omniskill file: ${join(homeDir, ".omniskill", "workflows", "git-workflow.json")}`,
      );
    } finally {
      console.log = originalLog;
      await rm(rootDir, { recursive: true, force: true });
  ...
  test("install supports a workflow alias source", async () => {
    ...
    try {
      ...
      expect(printedSkills).toEqual(["git-entry"]);
      expect(stripAnsiLines(logs)).toContain("Omniskill installed: git-workflow");
  ...
  test("validate supports a public git workflow source", async () => {
    ...
    try {
      ...
      expect(stripAnsiLines(logs)).toEqual([
        "Omniskill valid: git-workflow@0.1.0",
        "Steps: 1",
        "Skills: 1",
      ]);
      await expect(stat(checkoutDir)).rejects.toThrow();
  ...
  test("deps supports a public git workflow source", async () => {
    ...
    try {
      ...
      expect(stripAnsiLines(logs)).toEqual([
        "Omniskill dependencies: git-workflow",
        "- ./skills/git-entry",
      ]);
      await expect(stat(checkoutDir)).rejects.toThrow();
    } finally {
  ...
  test("onboard skips RTK and CodeGraph setup when declined", async () => {
    ...
    try {
      ...
      expect(stripAnsiLines(logs)).toContain("Omniskill onboard");
  ...
  test("onboard prints RTK setup guidance when RTK setup is accepted", async () => {
    ...
    try {
      ...
      expect(stripAnsiLines(logs)).toContain("Omniskill onboard complete");
      expect(stripAnsiLines(logs)).toContain("CodeGraph setup skipped");
    } finally {
      console.log = originalLog;
      await rm(rootDir, { recursive: true, force: true });
  ...
  test("uses the skills CLI by default before retrying missing external skill installs", async () => {
    ...
    await expect(
      stat(join(rootDir, ".omniskill", "workflows", "matt-bundle.json")),
    ).resolves.toBeTruthy();
  });
  ...
  test("uses the skills CLI before retrying each missing Superpowers workflow skill", async () => {
    ...
    await expect(
      stat(join(rootDir, ".omniskill", "workflows", "superpowers-bundle.json")),
    ).resolves.toBeTruthy();
  });
