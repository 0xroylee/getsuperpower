...
describe("snapshot history", () => {
  ...
  test("records and reads instruction context for pre and post phases", async () => {
    ...
    try {
      ...
      const rawEntries = (await readFile(join(rootDir, ".omniskills", "snapshots.jsonl"), "utf8"))
        .trim()
        .split("\n")
      ...
      expect(plan.actions).toEqual([
        {
          type: "restore",
          path: "notes.txt",
          source: join(rootDir, ".omniskills", "files", "snapshot-001", "pre", "notes.txt"),
        },
        {
          type: "delete",
          path: "created.txt",
  ...
  test("refuses unknown snapshots and missing stored copies", async () => {
    ...
    try {
      ...
      await rm(join(rootDir, ".omniskills", "files"), {
        recursive: true,
        force: true,
      });
...
async function writeSampleSnapshot(rootDir: string): Promise<void> {
  const snapshotDir = join(rootDir, ".omniskills");
