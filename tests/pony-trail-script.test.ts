...
describe("pony-trail shell helper", () => {
  test("records pre and post snapshots without requiring Python", async () => {
    ...
    try {
      ...
      const logPath = join(rootDir, ".omniskills", "snapshots.jsonl");
      ...
      await expect(
        stat(join(rootDir, ".omniskills/files", snapshotId, "pre", "notes.txt")),
      ).resolves.toBeTruthy();
      await expect(
        stat(join(rootDir, ".omniskills/files", snapshotId, "post", "notes.txt")),
      ).resolves.toBeTruthy();
      ...
      const sessionTree = await readFile(
        join(rootDir, ".omniskills", "sessions", "session-alpha", "tree.md"),
        "utf8",
      );
  ...
  test("captures instruction context only when opted in", async () => {
    ...
    try {
      ...
      const entries = (await readFile(join(rootDir, ".omniskills", "snapshots.jsonl"), "utf8"))
        .trim()
        .split("\n")
  ...
  test("python fallback captures schema-compatible instruction context when available", async () => {
    ...
    try {
      ...
      const entry = JSON.parse(
        (await readFile(join(rootDir, ".omniskills", "snapshots.jsonl"), "utf8")).trim(),
