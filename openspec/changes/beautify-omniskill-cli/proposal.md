# Proposal: Beautify Omniskill CLI Output

## Summary

Give the Omniskill CLI a more polished first impression: a branded
`OMNISKILL` ASCII logo, a warm welcome message, clearer command examples,
and a consistent color system for status output.

The direction should reference the product polish of Shopify CLI without
copying Shopify branding. Omniskill should feel sharper and more agentic:
...
```text
Usage: omniskill [options] [command]

Install, author, and inspect Omniskill skill trees.
```
...
- Add a CLI style guide document for Omniskill terminal output.
- Add a branded root help/welcome banner with an ASCII `OMNISKILL` logo.
...
Create `docs/omniskill-cli-design.md` as the style guide for terminal
...
Wire Commander root help through the theme helper so `omniskill --help`
...
- `omniskill --help` displays an ASCII logo whose visible text reads
  `OMNISKILL`.
...
- `docs/omniskill-cli-design.md` documents logo usage, palette, tone,
...
- Should the style guide live at `docs/omniskill-cli-design.md` as proposed,
...
- Should the ASCII logo appear only in root help, or also after successful
  installs?
- Should `omniskill` show the welcome banner when run with no command, or
