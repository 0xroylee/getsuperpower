...
Omniskill SHALL provide the reusable loop runtime from the CLI instead of
...
- **THEN** the generated runner invokes
  `omniskill loop status ./workflow.json --latest --json`
...
- **WHEN** the CLI runs `omniskill loop status <source> --latest --json`
- **THEN** returned action commands use `omniskill loop ...`
- **AND** they do not instruct the agent to run `node loop.mjs ...`

#### Scenario: direct Node runner has a clear missing-CLI failure

- **GIVEN** the generated `loop.mjs` runner cannot find `omniskill` on
  `PATH`
- **WHEN** a user runs `node loop.mjs status --latest --json`
- **THEN** the command fails with a clear message that the Omniskill CLI is
