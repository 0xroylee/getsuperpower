# Omniskill Command Spec

## ADDED Requirements

### Requirement: CLI Executes Looped Workflow Runs

The CLI SHALL expose a root `omniskill loop` command for controlling
...
- **WHEN** a user runs
  `omniskill loop start <source> --run smoke --json`
...
- **WHEN** a user runs `omniskill loop status <source> --latest --json`
...
- **WHEN** a user runs
  `omniskill loop log <source> --run smoke --type phase_result --message "done" --metadata '{"ok":true}' --json`
...
- **WHEN** a user runs `omniskill loop advance <source> --run smoke --json`
...
- **GIVEN** a workflow source declares `loop`
- **AND** run `smoke` exists
- **WHEN** a user runs `omniskill loop summary <source> --run smoke --json`
...
Omniskill commands where practical.

#### Scenario: user targets a local workflow directory

- **WHEN** a user runs
  `omniskill loop start examples/workflows/grilled-product-dev --json`
- **THEN** the CLI loads
...
- **WHEN** a user runs `omniskill loop start <source> --json`
- **THEN** the CLI fails with a clear message that the workflow is not looped

### Requirement: CLI Help Teaches Loop Subcommands

The CLI SHALL document loop execution as part of the primary Omniskill
command surface.
...
- **WHEN** a user runs `omniskill loop --help`
