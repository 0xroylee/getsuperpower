...
### Requirement: GetSuperpower Runtime Namespace

Active workflow runtime modules SHALL live under a GetSuperpower-named source
...
- **THEN** active references point at `src/runtimes/getsuperpower/`
...
GetSuperpower SHALL preserve install and validation behavior for workflows that
do not declare `loop`.

#### Scenario: workflow has no loop declaration

- **GIVEN** a workflow bundle without `loop`
- **WHEN** GetSuperpower validates or installs the workflow
