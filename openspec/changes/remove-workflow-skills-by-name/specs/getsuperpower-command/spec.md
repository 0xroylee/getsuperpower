# GetSuperpower Remove Command Spec
...
- **GIVEN** a workflow record exists at
  `<home>/.getsuperpower/workflows/release-review.json`
- **WHEN** a user runs
  `getsuperpower workflow remove release-review --home <home> --yes`
...
- **GIVEN** a workflow record exists at
  `<home>/.getsuperpower/workflows/release-review.json`
- **WHEN** a user runs `getsuperpower remove release-review --home <home> --yes`
...
- **GIVEN** a workflow record exists at
  `<project>/.getsuperpower/workflows/release-review.json`
- **WHEN** a user runs
  `getsuperpower remove release-review --dir <project> --home <home> --yes`
...
- **WHEN** a user runs `getsuperpower remove release-review --home <home> --dry-run`
...
- **WHEN** a user installs a workflow with
  `getsuperpower install <source> --home <home> --agents codex,claude,cursor`
...
- **WHEN** a user runs `getsuperpower remove missing-workflow --home <home>`
- **THEN** the CLI fails
- **AND** the error includes `GetSuperpower is not installed: missing-workflow`
...
- **AND** the manifest skills can be mapped to skill names from local skill
  ...
  names
- **WHEN** a user runs
  `getsuperpower remove legacy-workflow --home <home> --yes`
